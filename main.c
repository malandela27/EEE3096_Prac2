...
uint8_t bValues[6] = {0b10101010, 0b01010101, 0b11001100, 0b00110011, 0b11110000, 0b00001111};
...
int main(void)
{
  ...
  while (1)
  {
	...
	if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) == GPIO_PIN_RESET){
      ...
      // 1 second delay
      if((htim16.Instance -> ARR) == 499){
        HAL_TIM_Base_Stop(&htim16);// stop timer
        htim16.Instance -> ARR = 1000 - 1;// set ARR value to 1 second
        HAL_TIM_Base_Start(&htim16);// start timer
      }
      // 0.5 second delay
      else{
        HAL_TIM_Base_Stop(&htim16); // stop timer
        htim16.Instance -> ARR = 500 - 1;// set ARR value to 0.5 seconds
        HAL_TIM_Base_Start(&htim16);// start timer
      }
    }
...
void TIM16_IRQHandler(void)
{
   ...
   value = read_from_address(pos);    
      
   if(value == bValues[pos]){
     GPIOB -> ODR = value;
     pos = pos + 1;
   }
